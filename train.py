...
if __name__ == '__main__':
    ...
    if config.model.type=='subgraph_diffusion':
        ...
        noise_transforms= SubgraphNoiseTransform(config.model)
    ...
    def train(it):
        ...
        logger.info('[Train] Iter %05d | Loss %.2f | Loss(Global) %.2f | Loss(Local) %.2f | Grad %.2f | LR(Global) %.6f | LR(Local) %.6f |%s' % (
            it, loss.item(), loss_global.mean().item(), loss_local.mean().item(), orig_grad_norm, optimizer_global.param_groups[0]['lr'], optimizer_local.param_groups[0]['lr'], log_dir
        ))
